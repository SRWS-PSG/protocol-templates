# Build script for the network meta-analysis (NMA) review protocol template.
# Usage:
#   pwsh ./templates/NMA/build.ps1               # build all targets (docx, html) for EN
#   pwsh ./templates/NMA/build.ps1 -Target docx  # build a specific target
#   pwsh ./templates/NMA/build.ps1 -Lang en      # build a specific language only
#
# This template is English-only: there is no .ja.md master, so the ja pass is skipped.
# Outputs go to templates/NMA/build/ which is gitignored.
...
$sources = @{
    en = @{
        Src  = Join-Path $here 'protocol_template_for_nma_review.md'
        Base = 'protocol_template_for_nma_review'
    }
    ja = @{
        Src  = Join-Path $here 'protocol_template_for_nma_review.ja.md'
        Base = 'protocol_template_for_nma_review.ja'
    }
}
