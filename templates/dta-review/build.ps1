# Build script for the diagnostic test accuracy (DTA) review protocol template.
# Usage:
#   pwsh ./templates/dta-review/build.ps1               # build all targets (docx, html) for both EN and JA
#   pwsh ./templates/dta-review/build.ps1 -Target docx  # build a specific target
#   pwsh ./templates/dta-review/build.ps1 -Lang ja      # build a specific language only
#
# Outputs go to templates/dta-review/build/ which is gitignored.
...
$sources = @{
    en = @{
        Src  = Join-Path $here 'protocol_template_for_dta_review.md'
        Base = 'protocol_template_for_dta_review'
    }
    ja = @{
        Src  = Join-Path $here 'protocol_template_for_dta_review.ja.md'
        Base = 'protocol_template_for_dta_review.ja'
    }
}
